...
- If the repository is already available from the previous task, reuse it.Otherwise clone it: **Git repo** `https://github.com/RedHatTraining/AI26X-apps`
- Modify the file `AI26X-apps/intro/projects-data/projects-data-nb.ipynb` under the branch **`origin/RHOAI2.13`**
- Perform all tasks described in the notebook.
- In the cloned Jupyter notebook, the cell starting with comment **"saving THE JOBLIB Model file"** is updated to use joblib to save the model to the root of the cloned repo as a file named: `carido-model.joblib`.
...
## Lets ✅ **Complete the taks: Step-by-Step Solution**
...
```
cd AI26X-apps
git switch -c RHOAI2.13 origin/RHOAI2.13
...
#joblib.dump(model, model_path)   # Replace model with the actual model variable if different.
