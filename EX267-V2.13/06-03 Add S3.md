## Guided Exercise: Saving Trained Machine Learning Models
### How to prepare the lab?

```
lab start -t AI262 notebooks-collaboration
lab start -t AI265 serving-saving
echo <<EOF>> /tmp/.s3cfg
"AWS_ACCESS_KEY_ID": "minio",
"AWS_DEFAULT_REGION": "any",
"host_base": "minio-api-minio.apps.ocp4.example.com/%(bucket)/",
"AWS_SECRET_ACCESS_KEY": "minio123"
EOF
```

A **workbench** is already running `manage-resources-wb` under `manage-resources` project. 
- Your tasks are to modify the **workbench** with content mentioned in the `/tmp/.s3cfg` file and make sure all values are shown under bucket `saved-models`.
- You cloned already in the previous task, but if this is not the case then Clone the **Git repo** `https://github.com/RedHatTraining/AI26X-apps`
- Modify the file `AI26X-apps/intro/projects-data/projects-data-nb.ipynb` under the branch **`origin/RHOAI2.13`**
- Perform all the tasks whatever written on this notebook.
- In the cloned Jupyter notebook, the cell starting with comment **"saving THE JOBLIB Model file"** is updaed to use joblib to save the model to the root of the cloned repo as a file named: `carido-model.joblib`.
- Create a new branch called `experiment2` and save all the changes on this branch.
- Merge the `experiment2` feature branch into the `RHOAI2.13 branch`
---



### Solution
#save model
joblib.dump(ml_model,  "cardio-model.joblib")

- Click the **Git** icon, and change to the RHOAI2.13 branch by clicking **Current Branch** and selecting **origin/RHOAI2.13**.
- In the JupyterLab file browser, navigate to the `AI26X-apps/intro/projects-data/` directory and double click the `projects-data-nb.ipynb` notebook to open it.


`https://github.com/RedHatTraining/AI26X-apps`








---
---
---

# Red Hat OpenShift AI (RHOAI) Exam Task - S3 Data Connection & Notebook

**Closed environment Question / Task**

A workbench is already running `manage-resources-wb` under the `manage-resources` project.

### Tasks:
1. Modify the workbench and add the data connection named **`my-s3-connection`**.
2. Modify the file `AI26X-apps/intro/projects-data/projects-data-nb.ipynb` under the branch `origin/RHOAI2.13`.
3. Perform all the tasks whatever written on this notebook.
4. In the cloned Jupyter notebook, update the cell starting with comment **"saving THE JOBLIB Model file"** to use `joblib` to save the model to the **root of the cloned repo** as a file named: **`carido-model.joblib`**.

**Data Connection Details:**
```yaml
"AWS_ACCESS_KEY_ID": "minio"
"AWS_DEFAULT_REGION": "any"
"host_base": "minio-api-minio.apps.ocp4.example.com/saved-models"
"AWS_SECRET_ACCESS_KEY": "minio123"
```
---

### Solution

✅ **Complete Step-by-Step Solution**

## **1. Add Data Connection to the Workbench**

  1. Go to **OpenShift AI Dashboard → `Data Science Projects` → manage-resources**.
  2. Navigate to the Workbenches tab.
  3. Click the ⋮ menu next to `manage-resources-wb` → Edit workbench.
  4. Scroll to Connections section.
  5. Click Use a connection → Use existing connection.
  6. Select my-s3-connection from the list.
  7. Click Save and wait for the workbench to restart.



## **2. Clone & Checkout Repository**

  1. Open JupyterLab in manage-resources-wb.
  2. Click the Git icon in the left sidebar.
  3. Clone the repository containing AI26X-apps.
  4. Open a Terminal (File → New → Terminal).
  5. Run the following commands:
  
```
cd AI26X-apps
git checkout origin/RHOAI2.13
git pull
```



## **3. Open and Complete the Notebook**
- Open the notebook: **`AI26X-apps/intro/projects-data/projects-data-nb.ipynb`**
- Run all cells sequentially as instructed in the notebook.

![S3 Information would be blank ](image-1.png)
```python
key_id = os.getenv("AWS_ACCESS_KEY_ID")
secret_key = os.getenv("AWS_SECRET_ACCESS_KEY")
endpoint = os.getenv("AWS_ENDPOINT")
bucket_name = os.getenv("AWS_S3_BUCKET")
```

## **4. Update the Joblib Save Cell**
- Replace the cell with comment "saving THE JOBLIB Model file" with the following code:
```python
import joblib
import os

# Save the trained model (adjust variable name if different)
#model_path = "carido-model.joblib"
#joblib.dump(model, model_path)   # 'model' should be your trained model object

joblib.dump(ml_model, carido-model.joblib)   # 'model' should be your trained model object
```


# Final Checklist

 Data Connection my-s3-connection attached to workbench
 Notebook checked out on correct branch
 All notebook cells executed successfully
carido-model.joblib created in root of cloned repo
 No errors related to S3 / joblib



 ---
 ---
 ---


 # Manage Resources Workbench Solution

## Task

A workbench named `manage-resources-wb` is already running in the `manage-resources` project.

Complete the following:

1. Modify the workbench and add the data connection named `my-s3-connection`.
2. Modify the file:

   ```text
   AI26X-apps/intro/projects-data/projects-data-nb.ipynb
   ```

   from branch:

   ```text
   origin/RHOAI2.13
   ```

3. Perform all tasks written in that notebook.
4. In the cloned Jupyter notebook, update the cell starting with:

   ```python
   # saving THE JOBLIB Model file
   ```

   so that it uses `joblib` to save the model to the root of the cloned repository as:

   ```text
   carido-model.joblib
   ```

## Data connection details

Use the following values:

```json
{
  "AWS_ACCESS_KEY_ID": "minio",
  "AWS_DEFAULT_REGION": "any",
  "host_base": "minio-api-minio.apps.ocp4.example.com/saved-models",
  "AWS_SECRET_ACCESS_KEY": "minio123"
}
```

---

## Solution

### 1. Log in to OpenShift

Log in to the cluster from a terminal where the `oc` CLI is available.

```bash
oc login --server=<OPENSHIFT_API_URL> -u <USERNAME> -p <PASSWORD>
```

Switch to the required project:

```bash
oc project manage-resources
```

Verify that the workbench exists:

```bash
oc get notebook manage-resources-wb -n manage-resources
```

---

### 2. Create the data connection secret

Create a secret named `my-s3-connection` in the `manage-resources` project.

```bash
oc create secret generic my-s3-connection \
  -n manage-resources \
  --from-literal=AWS_ACCESS_KEY_ID=minio \
  --from-literal=AWS_SECRET_ACCESS_KEY=minio123 \
  --from-literal=AWS_DEFAULT_REGION=any \
  --from-literal=host_base=minio-api-minio.apps.ocp4.example.com/saved-models
```

Add the labels used by the RHOAI dashboard for data connections:

```bash
oc label secret my-s3-connection \
  -n manage-resources \
  opendatahub.io/dashboard=true \
  opendatahub.io/managed=true \
  --overwrite
```

Verify the secret:

```bash
oc get secret my-s3-connection -n manage-resources --show-labels
```

---

### 3. Attach the data connection to the workbench

Patch the `manage-resources-wb` workbench so the notebook pod receives the data connection values as environment variables.

```bash
oc patch notebook manage-resources-wb \
  -n manage-resources \
  --type='json' \
  -p='[
    {
      "op": "add",
      "path": "/spec/template/spec/containers/0/envFrom/-",
      "value": {
        "secretRef": {
          "name": "my-s3-connection"
        }
      }
    }
  ]'
```

If the command fails because `envFrom` does not exist yet, initialize it first:

```bash
oc patch notebook manage-resources-wb \
  -n manage-resources \
  --type='json' \
  -p='[
    {
      "op": "add",
      "path": "/spec/template/spec/containers/0/envFrom",
      "value": []
    }
  ]'
```

Then run the previous patch command again.

Restart the workbench pod so the environment variables are loaded:

```bash
oc delete pod -n manage-resources -l notebook-name=manage-resources-wb
```

Wait for the workbench pod to return to `Running`:

```bash
oc get pods -n manage-resources
```

Confirm that the data connection values are available inside the notebook pod:

```bash
POD=$(oc get pod -n manage-resources -l notebook-name=manage-resources-wb -o jsonpath='{.items[0].metadata.name}')

oc exec -n manage-resources "$POD" -- env | grep -E 'AWS_ACCESS_KEY_ID|AWS_DEFAULT_REGION|host_base'
```

---

### 4. Open the Jupyter workbench

Open the RHOAI dashboard, go to the `manage-resources` project, and open the running workbench named:

```text
manage-resources-wb
```

Open a terminal inside JupyterLab.

---

### 5. Clone the repository and switch to the required branch

In the JupyterLab terminal, clone the repository if it is not already cloned.

```bash
git clone <REPOSITORY_URL> AI26X-apps
```

Move into the repository:

```bash
cd AI26X-apps
```

Fetch all remote branches:

```bash
git fetch --all
```

Checkout the required branch:

```bash
git checkout -B RHOAI2.13 origin/RHOAI2.13
```

Verify the branch:

```bash
git branch --show-current
```

Expected output:

```text
RHOAI2.13
```

---

### 6. Open the notebook

Open this notebook in JupyterLab:

```text
AI26X-apps/intro/projects-data/projects-data-nb.ipynb
```

From the repository root, the path is:

```text
intro/projects-data/projects-data-nb.ipynb
```

Run the notebook cells and complete all instructions written inside the notebook.

---

### 7. Update the joblib model-saving cell

Find the cell that starts with:

```python
# saving THE JOBLIB Model file
```

Update it so the model is saved to the root of the cloned repository as:

```text
carido-model.joblib
```

Use this code pattern:

```python
# saving THE JOBLIB Model file

import joblib
from pathlib import Path

# Save the model to the root of the cloned AI26X-apps repository.
# If the notebook is running from AI26X-apps/intro/projects-data,
# parents[2] points to the AI26X-apps repo root.
repo_root = Path.cwd()

# Walk upward until the AI26X-apps repository root is found.
while repo_root.name != "AI26X-apps" and repo_root.parent != repo_root:
    repo_root = repo_root.parent

model_path = repo_root / "carido-model.joblib"

joblib.dump(model, model_path)

print(f"Model saved to: {model_path}")
```

If the trained model variable in the notebook is not named `model`, replace `model` with the actual trained model variable name used in the notebook, for example:

```python
joblib.dump(clf, model_path)
```

or:

```python
joblib.dump(rf_model, model_path)
```

---

### 8. Run the notebook

In JupyterLab, run all notebook cells from top to bottom:

```text
Kernel -> Restart Kernel and Run All Cells
```

Confirm that the model file was created at the root of the cloned repository:

```bash
cd ~/AI26X-apps 2>/dev/null || cd /opt/app-root/src/AI26X-apps
ls -l carido-model.joblib
```

Expected result:

```text
carido-model.joblib
```

---

### 9. Optional: verify with Python

Run:

```bash
python - <<'PY'
from pathlib import Path

model_file = Path("carido-model.joblib")
print(model_file.resolve())
print("Exists:", model_file.exists())
print("Size:", model_file.stat().st_size if model_file.exists() else "missing")
PY
```

Expected output should show:

```text
Exists: True
```

---

## Final validation checklist

Use this checklist before submission:

- [ ] The `manage-resources` project is selected.
- [ ] The workbench `manage-resources-wb` exists and is running.
- [ ] The data connection secret `my-s3-connection` exists.
- [ ] The data connection contains:
  - [ ] `AWS_ACCESS_KEY_ID=minio`
  - [ ] `AWS_SECRET_ACCESS_KEY=minio123`
  - [ ] `AWS_DEFAULT_REGION=any`
  - [ ] `host_base=minio-api-minio.apps.ocp4.example.com/saved-models`
- [ ] The workbench has been restarted after adding the data connection.
- [ ] The repository is checked out to `RHOAI2.13` from `origin/RHOAI2.13`.
- [ ] The notebook `intro/projects-data/projects-data-nb.ipynb` has been opened and completed.
- [ ] The cell beginning with `# saving THE JOBLIB Model file` uses `joblib.dump(...)`.
- [ ] The model is saved at the root of the cloned repo as `carido-model.joblib`.

---

## Notes

The required filename is written exactly as requested:

```text
carido-model.joblib
```

Do not rename it to `cardio-model.joblib` unless the grading instructions explicitly require that spelling.




---
---
---
```
```

# Guided Exercise: Saving Trained Machine Learning Models

## Objective
Learn how to save a trained machine learning model using **Joblib** in a Red Hat OpenShift AI (RHOAI) environment and manage changes using Git branching and merging.

---

## Lab Preparation (Instructor / Environment Setup)

Run the following commands to prepare the lab environment:

```bash
lab start -t AI262 notebooks-collaboration
lab start -t AI265 serving-saving

# Create S3 configuration file
cat <<EOF > /tmp/.s3cfg
[AWS]
AWS_ACCESS_KEY_ID = minio
AWS_SECRET_ACCESS_KEY = minio123
AWS_DEFAULT_REGION = any
host_base = minio-api-minio.apps.ocp4.example.com
EOF
```

Pre-existing Setup:

A workbench named manage-resources-wb is already running in the manage-resources project.
Data Connection my-s3-connection should be pre-created with the following values:
AWS_ACCESS_KEY_ID: minio
AWS_SECRET_ACCESS_KEY: minio123
AWS_DEFAULT_REGION: any
Bucket: saved-models



Student Tasks

Attach the my-s3-connection data connection to the workbench manage-resources-wb.
Clone the Git repository (if not already cloned):
https://github.com/RedHatTraining/AI26X-apps
Checkout the branch origin/RHOAI2.13.
Open and complete all tasks in the notebook:
AI26X-apps/intro/projects-data/projects-data-nb.ipynb
Update the cell starting with the comment "saving THE JOBLIB Model file" to save the trained model as cardio-model.joblib in the root of the cloned repository using joblib.
Create a new branch called experiment2, commit your changes, and merge it into the RHOAI2.13 branch.


Step-by-Step Solution
1. Attach Data Connection to Workbench

Go to OpenShift AI Dashboard → Data Science Projects → manage-resources.
Navigate to the Workbenches tab.
Click the ⋮ (kebab menu) next to manage-resources-wb → Edit workbench.
Scroll down to the Connections section.
Click Use a connection → Use existing connection.
Select my-s3-connection from the list.
Click Save and wait for the workbench to restart.

2. Clone and Checkout the Repository
Open the workbench JupyterLab and run the following commands in a Terminal:

```
cd ~
git clone https://github.com/RedHatTraining/AI26X-apps.git 2>/dev/null || echo "Repository already exists"
cd AI26X-apps
git checkout RHOAI2.13
git pull origin RHOAI2.13
```
3. Open and Complete the Notebook

In JupyterLab file browser, navigate to: `AI26X-apps/intro/projects-data/`

Double-click projects-data-nb.ipynb to open it.
Run all cells sequentially as instructed in the notebook.

Expected Environment Variables (available via attached Data Connection):

```python
import os
print("AWS_ACCESS_KEY_ID:", os.getenv("AWS_ACCESS_KEY_ID"))
print("AWS_S3_BUCKET:", os.getenv("AWS_S3_BUCKET"))
```

4. Update the Joblib Save Cell
Locate the cell with the comment "saving THE JOBLIB Model file" and replace its content with the following corrected code:
Python
```python
import joblib

# Save the trained model to the root of the cloned repository
model_path = "cardio-model.joblib"
joblib.dump(ml_model, model_path)  # Use the correct model variable from your notebook

print(f"✅ Model successfully saved as {model_path} in the repository root.")
```

Note: Ensure the variable ml_model matches the trained model object created earlier in the notebook. Change it if your model variable has a different name (e.g., model, best_model, etc.).
5. Git Branching and Merging
Run the following commands in the Terminal:

```bash
# Create new feature branch
git checkout -b experiment2

# Stage and commit changes
git add ../cardio-model.joblib projects-data-nb.ipynb
git commit -m "feat: save trained model using joblib - experiment2"

# Switch to main branch and merge
git checkout RHOAI2.13
git merge experiment2 --no-ff -m "Merge experiment2: Saved cardio model with joblib"

# Push changes (optional, if required by lab)
git push origin RHOAI2.13
```


Final Verification Checklist

 Data Connection my-s3-connection is attached to the workbench.
 Notebook is checked out on branch RHOAI2.13.
 File cardio-model.joblib exists in the root of the AI26X-apps repository.
 All notebook cells executed without errors.
 Changes merged successfully from experiment2 branch into RHOAI2.13.